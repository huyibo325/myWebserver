...
const char*doc_root="/home/ubuntu/webserver/root";
...
void http_conn::initmysql_result(connection_pool*connPool){
    ...
    //在user表中检索username,password,浏览器的端的输入
...
//修改文件描述符,重置EPOLLONESHOT事件，以确保下次可读
...
//如果目标文件存在、对所有用户可读，且不是目录，则使用mmap将其映射到内存
...
http_conn::HTTP_CODE http_conn::do_request(){
    ...
    if(cgi==1&&(*(p+1)=='2')||*(p+1)=='3'){
        ...
        //如果是登录，之间判断
        //若浏览器端输入的用户名和密码在表中可用查找到，返回1，否则返回0
        else if(*(p+1)=='2'){
